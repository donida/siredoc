class EstadosController < ApplicationController
  ...
  # GET /estados.json
  def index
    @estados = Estado.paginate(:page => params[:page], :order => 'nome asc', :per_page => 10)
#    @estados = Estado.all
  end
  ...
  def create
    ...
    respond_to do |format|
      if @estado.save
        format.html { redirect_to @estado, notice: 'Estado foi criado(a) com sucesso.' }
  ...
  def update
    respond_to do |format|
      if @estado.update(estado_params)
        format.html { redirect_to @estado, notice: 'Estado foi atualizado(a) com sucesso.' }
